...
export default class Basemodel {
    ...
    static get(id, filters = {}) {
        ...
        return api.getResources(id, this, filters).then(response => {
    ...
    addNested(nestedRes) {
        return api.addNetesResource(this, nestedRes).then(data => {
            switch (data.status) {
                case 200:
                    return { success: true, data: data.data };
                case 422:
    ...
    _create() {
        return api.createdResource(this).then(data => {
            switch (data.status) {
                case 200:
            ...
            switch (parseInt(error.response.status)) {
                case 422:
                    this._parseErrors(erro.response.data)
                break;
            }
    ...
    extractFields(baseModel) {
        const data = {};
        this.recursiveEctractFields(this, data);
        return data;
    }
